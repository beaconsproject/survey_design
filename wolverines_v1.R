...
ui = dashboardPage(
  dashboardHeader(title = "Wolverines Survey v1"),
  dashboardSidebar(
    sidebarMenu(
        menuItem("Sampling Design", tabName = "fri", icon = icon("th"))
    ),
    h5('(1) View covariates'),
    selectInput("inv", label = "Select feature:", choices = names(factors)[3:22], selected='merge100_pct'),
    ...
    hr(),
    h5('(3) Select grids'),
    sliderInput("size", label="Sample size per strata:", min=0, max=100, value=35, step=5, ticks=FALSE),
...
server = function(input, output) {
    ...
    output$map1 <- renderLeaflet({
        ...
        if (input$clustButton) {
            m <- tm_shape(clusters()) + tm_fill('clusters', palette='Set1', alpha=1, group="Clusters")
        } else {
            m <- tm_shape(factors) + tm_fill(input$inv, palette=pal, style=input$style, alpha=1, colorNA=NULL, group=input$inv, title=input$inv)
        }
        m <- m + tm_shape(grid) + tm_borders(group='Grid') +
            tm_shape(settlement) + tm_fill(col='blue', alpha=0.5, group="Settlement lands") +
            tm_shape(linear) + tm_lines(col='black', alpha=1, group="Linear features") +
            tm_shape(areal) + tm_fill(col='red', alpha=1, group="Areal features") +
            tm_shape(thtt) + tm_borders(col='yellow', lwd=2, group="TH traditional territory")
        if (input$goButton) {
            n1rnd3 <- st_as_sf(terra::spatSample(terra::vect(n1()), 3, strata='id'))
            #n2rnd3 <- st_as_sf(terra::spatSample(terra::vect(n2()), 3, strata='id'))
            m <- m + tm_shape(n1()) + tm_borders(col='green', lwd=2, group="Simple random") +
                ...
                tm_shape(n1rnd3) + tm_dots(size=0.02, col='black', group="Camera traps (stratified)")
        }
