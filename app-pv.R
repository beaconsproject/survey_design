...
server = function(input, output) {
    ...
    output$map1 <- renderLeaflet({
        ...
        m <- m + tm_shape(grid) + tm_borders() +
            ...
            tm_shape(thtt) + tm_borders(col='yellow', lwd=2, group="TH tradional territory")
        if (input$goButton) {
        ...
        lf <- tmap_leaflet(m)
        lf %>% leaflet::hideGroup(c("Areal disturbances","Linear disturbances","Areal features","Linear features","Elevation","Stratified random","TH tradional territory","Settlement lands","Camera traps (stratified)"))
    })
